...
pytest tests/platforms/test_spatialest.py::TestSpatialestRecordcardParsesFields   # single class
pytest tests/platforms/test_spatialest.py::TestFieldNameVariants::test_parse_sqft_sfla  # single test
...
### 1. Platforms and jurisdiction drivers
A *platform* is vendor software serving many jurisdictions (Spatialest, Tyler EagleWeb, Aumentum, Esri ArcGIS REST); a *jurisdiction* is the assessor's territory (county, parish, independent city). Platform code lives in `platforms/` and carries zero jurisdiction knowledge; bespoke per-jurisdiction flows live in `jurisdictions/<country>/<state>/` and import the platform primitives (one-way dependency). Most jurisdictions never get a file — a county on a config-driven platform is just a registry entry. Shared building blocks live in `core/` (`statuses.py`, `matching.py`, `network.py` — the HTTPS/SSRF boundary).

Every client exposes the same duck-typed contract (`AssessorAdapter` Protocol in `platforms/__init__.py`): `lookup(address, ...)` and optionally `lookup_by_parcel(parcel_id, ...)`, each returning a dict whose `status` is one of `success`, `not_found`, `ambiguous`, `timeout`, `api_error`, `parse_error`, `invalid_address`, `invalid_parcel` (constants in `core/statuses.py`). On `success` the dict carries the standard record keys (`owner`, `legal`, `parcel_number`, `above_grade_sqft`, `basement_sqft`, `beds`, `baths`, `year_built`, `tax_amount`, `assessed_value`, `market_value`, `latitude`, `longitude`, `assessor_url`, `neighborhood`, `zoning`, `lot_size_sqft`, `garage_area_sqft`, ...). Each client class declares a `capabilities` dict (`parcel_lookup`, `building_fields`) — consult it instead of `hasattr`.

- `platforms/spatialest.py` — `SpatialestClient`, the default. Handles the multi-county Spatialest hosted platform (El Paso, Denver, Douglas, and any unregistered county via slug fallback). Two-step flow: POST `/api/v2/search` → property id, then GET `/api/v1/recordcard/{id}`. **Spatialest returns HTTP 419 when a request lacks a page/CSRF token** — `_spatialest_request` catches 419 and replays via `_spatialest_request_with_page_token`, which scrapes the `csrf-token` meta tag and reuses the session cookie. Field extraction is defensive: `_extract_all_fields` recursively flattens sections (handles both El Paso's dict-keyed and Denver's list-indexed formats), then parsers like `_parse_sqft` / `_parse_lot_area` try a long ordered list of possible field-name aliases because each county names the same datum differently.
- `platforms/aumentum.py` — `JeffcoClient`, the Aumentum REST platform (base-URL-configurable; Jefferson County is the default/first deployment).
- `jurisdictions/us/co/arapahoe.py` — `ArapahoeClient`, ArcGIS driver on the county's self-hosted MapServer (geocode → parcel/owner/building layers); a lookup is sequential but callers must still respect rate limits.
- `jurisdictions/us/co/adams.py` — `AdamsClient`, ArcGIS driver (ArcGIS Online FeatureServer). The compact reference example for adding a new ArcGIS jurisdiction driver; shared Esri REST primitives (and the one mockable HTTP seam, `arcgis_get`) live in `platforms/arcgis.py`.
- `platforms/eagleweb.py` — `EagleWebClient`, Tyler's "EagleWeb / taxweb" JSP app (Clear Creek). **No JSON API exists** — this drives the public *guest* session (GET `/web/` for a cookie → POST `/web/loginPOST.jsp` with `guest=true` → POST `/taxweb/results.jsp` search → GET `/taxweb/account.jsp`) and parses HTML. Building data (GLA/beds/baths/year) lives only on the per-account **residential improvement** detail page, reached via a *dynamic* `doc=DOC…` id that must be discovered from the summary page, not hardcoded. GLA = the "Square Feet" column of the Abstract-Code/Areas table; tax is computed `assessed × mill levy / 1000` split school/non-school. Registry entry carries a `base` URL. This is the reference for scraping-based platforms.
- `jurisdictions/us/co/statewide.py` — `CoParcelClient`, the Colorado **statewide** public-parcel ArcGIS layer (`gis.colorado.gov`, ~40 counties). Clean JSON but **parcel/owner/legal/value/land only — no building characteristics**, so beds/baths/GLA/year are always `None` with a `data_note`. This is the tier-2 *baseline* for auto-discovery, not a full assessor client.
...
`discover_county(county, state)` maps an unknown county to a registry entry via a best-data-first ladder: **tier 1** = full building data detectable by URL (Spatialest slug probe → EagleWeb CO host convention `assessor.co.<hyphenated-county>.<state>.us/eagleassessor`); **tier 2** = the CO statewide parcel API baseline (membership set `_CO_PARCEL_COUNTIES`). Probes fail closed (any error → "not this platform"). Hits are returned as normalized registry entries and cached to a per-user registry (`~/.config/assessor-lookup/county_registry.json`, override `ASSESSOR_LOOKUP_HOME`) which `registry.load_registry()` merges over the packaged defaults by canonical identity. `checker._get_client` calls discovery inline for unregistered counties and persists the hit; the `discover` CLI command does the same explicitly.

### 2. Registry + dispatch + checker
- `registry.py` + `county_registry.json` — the jurisdiction model. Canonical keys are `"{COUNTRY}/{STATE}/{kind}:{name-slug}"` (e.g. `"US/CO/county:el-paso"`; `kind` also covers `parish`, `independent_city`, ...) mapping to `{jurisdiction, platform, config, access}`. The loader reads the legacy `"CO:El Paso"` key format and legacy platform aliases (`jeffco`, `adams`, `arapahoe`, `co_parcel_api`) forever, normalizing on load; all writes emit the new format. Credentials never live in the registry (`access` is a mode/reference only). Unregistered counties fall back to Spatialest with `slug = county.lower().replace(" ", "")`.
- `checker._get_client(county, state)` — the dispatcher. Resolves via `registry.resolve()` (exact slug match, then **state-scoped** partial-name match spanning kinds, then discovery, then Spatialest fallback) and constructs the client through the `PLATFORMS` dict in `platforms/__init__.py` (`build_client(entry)`). **`PLATFORMS` is the single registration point** — a new platform is one module + one dict line; a new ArcGIS jurisdiction is a driver module named in `config.driver`.
...
## Predefined agents & skills (`.claude/`)
On clone, Claude Code auto-discovers agent and skill definitions that make the repo turnkey — an LLM picks up named roles instead of improvising. `.claude/agents/`: **coordinator** (entry point; routes work, enforces API-first + honest coverage reporting), **explorer** (read-only; maps an unknown county's site, API-first), **reviewer** (adversarial verify of a new client vs the live site + golden), **county-onboarder** (probe + onboard a user's counties). `.claude/skills/`: **onboard-locale**, **appraisal-check**, **add-county**. These are the same workflows the MCP prompts encode, expressed as invocable agents/skills; the MCP operating manual points at them by name. `tests/test_agents_skills.py` guards their frontmatter (name matches file/dir, description/body present) so a typo can't silently make one undiscoverable — no YAML dependency. The `.claude/` files are repo-level (not shipped in the wheel).
...
`FastMCP` server (optional `[mcp]` extra, Python 3.10+; entry point `assessor-lookup-mcp`, and `python -m assessor_lookup.mcp_server`) exposes the package as an agent-ready service. It wraps the same public functions and packaged `assessor_lookup.harness`, so MCP harness tools work from both a clone and an installed wheel. Tools: `lookup_property`, `check_mls_csv`, `list_counties`, `discover_county`, `probe_county`, `onboard_county`, `run_regression`, `benchmark`. Resources: `assessor://operating-manual`, `assessor://architecture`, `assessor://counties`, `assessor://golden-records`, `assessor://harness-guide`. Prompts: `appraisal_check`, `onboard_locale`, `add_new_county`. The repo's `tests/harness.py` is a thin CLI wrapper. Bundled `.mcp.json` lets Claude Code auto-discover it. Tests: `tests/test_mcp_server.py` (offline, `pytest.importorskip("mcp")`).
...
The operational risk of this package is county websites changing silently. `assessor_lookup/golden_records.json` pins privacy-minimized stable fields per county (captured live via `--capture`); `python tests/harness.py` re-fetches each and compares.
...
## Adding a jurisdiction (county / parish / city)
1. If it's on a config-driven platform (Spatialest, EagleWeb, Aumentum), add a registry entry: `{"US/CO/county:name": {"jurisdiction": {...}, "platform": "spatialest", "config": {"slug": "..."}}}` — or rely on discovery / the slug fallback. No code.
2. If it needs a bespoke ArcGIS flow, add a driver module at `jurisdictions/<country>/<state>/<name>.py` exposing `build(entry, timeout=, verbose=)` (mirror `jurisdictions/us/co/adams.py`), and reference it from the entry's `config.driver`.
3. For a brand-new platform, add one module in `platforms/` implementing the `AssessorAdapter` contract (status dict, never raise, `capabilities`), and register it with one line in the `PLATFORMS` dict.
4. Tests mock at the HTTP boundary (`_spatialest_request`, `platforms.arcgis.arcgis_get`, or `JeffcoClient._get_endpoint`) — never hit the network in non-`network` tests. Add fixtures like `MOCK_RECORDCARD_RESPONSE`.
